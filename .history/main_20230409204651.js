...
function move(){
    const zombies = document.querySelector('.zombie');
    console.log(zombies);
    for(let i =0; i<zombies.length; i++){
        let x = 0;
        let y = 0;

        setInterval(moveZombie(zombies[i]),1000,);
        ...
        function moveZombie(zombie){
            ...
            x = newX;
            y = newY;
            zombie.style.transition = 'transform 300ms';
            zombie.style.transform = `translate(${deltaX}px,${deltaY}px)`;
        }
    }
}
